import { IBoardAddRequest, IBoardResponse, IBoardUpdateRequest } from "@/types/IBoard";
...
export async function updateBoardApi(board: IBoardUpdateRequest): Promise<IBoardResponse> {
    const response = await axios.put<IBoardResponse>(`${API_BASE_URL}/api/boards`, board);
    return response.status === HttpStatusCode.Ok
        ? response.data as IBoardResponse
        : {} as IBoardResponse;
}
